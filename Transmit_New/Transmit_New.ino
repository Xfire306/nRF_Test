...
if (role == 1)  {
    ...
    Serial.println(F("Now sending"));
     ...
     start_time[x] = micros();                             // Take the time, and send it.  This will block until complete
     if (!radio.write( &start_time[x], sizeof(unsigned long) )){
    ...
    if ( timeout ){                                             // Describe the results
        ...
        radio.read( &got_time[x], sizeof(unsigned long) );
...
//        Serial.println((end_time-start_time)/1000);
         ...
         average_round_trip=average_round_trip/1000;
         Serial.print(average_round_trip);
